...
const chartColors = ['hsl(var(--chart-primary))', 'hsl(var(--chart-secondary))', 'hsl(var(--success))', 'hsl(var(--warning))', 'hsl(var(--destructive))', 'hsl(187, 60%, 60%)', 'hsl(262, 60%, 65%)'];
const timeRanges = [{
  label: '6M',
  months: 6
}, {
...
export function HistoricalChart() {
  const [visibleIndicators, setVisibleIndicators] = useState<string[]>(indicators.map(i => i.id));
  ...
  const {
    ...
    indicators.forEach(indicator => {
      ...
      const normalized = showNormalized ? normalizeData(slicedData) : slicedData.map(p => ({
        ...p,
    ...
    const merged = Array.from(dataMap.entries()).map(([date, values]) => ({
      date,
      ...values,
      ...
      <Card className="border-border/50 bg-card">
        <CardHeader className="pb-4">
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {showNormalized ? 'Variação percentual relativa ao início do período' : 'Valores absolutos dos indicadores'}
                </p>
              </div>
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2">
                  <Switch id="normalize" checked={showNormalized} onCheckedChange={setShowNormalized} />
                  <Label htmlFor="normalize" className="text-xs">
                    Normalizado
                  </Label>
                </div>
                <div className="flex gap-1 rounded-lg bg-muted p-1">
                  {timeRanges.map(range => <Button key={range.months} variant="ghost" size="sm" onClick={() => setTimeRange(range.months)} className={cn('h-7 px-3 text-xs', timeRange === range.months && 'bg-background shadow-sm')}>
                      {range.label}
                    </Button>)}
                </div>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <Button variant="outline" size="sm" onClick={toggleAll} className="h-7 gap-1.5 border-border/50 px-2.5 text-xs">
                {visibleIndicators.length === indicators.length ? <><EyeOff className="h-3 w-3" /> Ocultar todos</> : <><Eye className="h-3 w-3" /> Mostrar todos</>}
              </Button>
              <div className="h-4 w-px bg-border" />
              {indicators.map((indicator, index) => {
              const trend = trendSummary.find(t => t.id === indicator.id);
              const isVisible = visibleIndicators.includes(indicator.id);
              return <Button key={indicator.id} variant="outline" size="sm" onClick={() => toggleIndicator(indicator.id)} className={cn('h-7 gap-1.5 border-border/50 px-2.5 text-xs transition-all', isVisible && 'border-primary/50 bg-primary/10')}>
                    <span className="h-2 w-2 rounded-full transition-colors" style={{
                  backgroundColor: isVisible ? chartColors[index % chartColors.length] : 'hsl(var(--muted-foreground))'
                }} />
                    {indicator.shortName}
                    {trend && isVisible && <span className={cn('flex items-center gap-0.5', getTrendColor(trend.trend, indicator.id))}>
        ...
        <CardContent>
          <div className="h-96">
            <ResponsiveContainer width="100%" height="100%">
              ...
              bottom: 0
                ...
                stroke: 'hsl(var(--border))'
              }} width={50} tickFormatter={value => showNormalized ? `${value.toFixed(0)}%` : value.toFixed(1)} />
                {showNormalized && <ReferenceLine y={0} stroke="hsl(var(--muted-foreground))" strokeDasharray="5 5" strokeWidth={1} />}
                ...
                return <Line key={indicator.id} type="monotone" dataKey={indicator.id} stroke={chartColors[index % chartColors.length]} strokeWidth={2} dot={false} activeDot={{
                  r: 4,
                  strokeWidth: 2
                }} name={indicator.id} isAnimationActive={true} animationDuration={800} />;
              })}
              </LineChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
            {indicators.filter(i => visibleIndicators.includes(i.id)).map((indicator, index) => {
            const trend = trendSummary.find(t => t.id === indicator.id);
